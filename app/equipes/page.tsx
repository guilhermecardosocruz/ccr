    ...
    <main className="container-page space-y-6">
      <header className="card p-4">
        ...
        <div className="flex gap-2">
          <input value={name} onChange={e=>setName(e.target.value)} className="border rounded-md px-3 py-2 w-full" placeholder="Nome da equipe" />
          <button onClick={add} className="px-3 py-2 border rounded-md">Adicionar</button>
        </div>
      </header>
      <section className="card p-0 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50"><tr><th className="text-left px-3 py-2">Equipe</th><th className="w-40 px-3 py-2">Ações</th></tr></thead>
          <tbody>
