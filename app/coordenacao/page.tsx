...
function Inner(){
  ...
  const byTeam = useMemo(()=>{ const m=new Map<string,R[]>(); for(const r of runs){ if(!m.has(r.team)) m.set(r.team,[]); m.get(r.team)!.push(r);} for(const a of m.values()) a.sort((x,y)=>x.at-y.at); return m;},[runs]);
