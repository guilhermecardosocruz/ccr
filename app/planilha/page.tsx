...
const MARC: Record<Attempt,number> = {1:100,2:75,3:50};
const MINA: Record<Attempt,number> = {1:1.5,2:1.25,3:1.15};
const makeState = () => Object.fromEntries(Object.entries(DESAFIOS).map(([k,v])=>[k,Array(v.rows).fill(false)])) as Record<DKey,boolean[]>;
...
function Planilha(){
  ...
  useEffect(()=>{ if(!running) setTimeLeft(durationMin*60); },[durationMin,running]);
  useEffect(()=>{ if(!running) return; const id=setInterval(()=>setTimeLeft(t=>t>0?t-1:0),1000); return ()=>clearInterval(id); },[running]);

  const canStart = !!selected || running;
  const startPause = ()=>{ if(!selected) return; if(timeLeft<=0) setTimeLeft(durationMin*60); setRunning(r=>!r); };
  ...
  const toggle=(d:DKey,i:number)=>{ if(!canScore) return; setTab(p=>{const n={...p}; n[d]=[...n[d]]; n[d][i]=!n[d][i]; return n;}); };
  const pick=(t:Attempt,q:0|1|2)=>{ if(!canScore) return; setMark(p=>({...p,[t]:p[t]===q?0:q})); };

  const somaCol:Record<DKey,number> = useMemo(()=>{ const r:any={}; (Object.keys(DESAFIOS) as DKey[]).forEach(k=>r[k]=tab[k].reduce((a,on)=>a+(on?DESAFIOS[k].points:0),0)); return r; },[tab]);
  const somaDes = (Object.keys(DESAFIOS) as DKey[]).reduce((a,k)=>a+somaCol[k],0);
  const somaMar = ([1,2,3] as Attempt[]).reduce((a,t)=>a+(mark[t]?MARC[t]:0),0);
  ...
  return (
    <main className="container-page space-y-6">
      <header className="card p-3 md:p-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <select className="px-2 py-1.5 border rounded-md bg-white" value={durationMin} onChange={e=>setDurationMin(Number(e.target.value))} disabled={running} title={running?"Pause para alterar":"Selecione a duração"}>
              {[2,3,4,5,6,7,8,9,10].map(m=><option key={m}>{m}</option>)}
            </select>
            <div className="text-3xl font-mono">{mmss(timeLeft)}</div>
            <button onClick={startPause} className={`px-3 py-2 border rounded-md ${!canStart?"opacity-50 cursor-not-allowed":""}`} disabled={!canStart}>{running?"Pausar": timeLeft<=0?"Reiniciar":"Iniciar"}</button>
            <button onClick={penalty} disabled={!running||timeLeft<=0||!selected} className={`px-3 py-2 border rounded-md ${running&&timeLeft>0&&selected?"hover:bg-gray-50":"opacity-50 cursor-not-allowed"}`}>Penalidade -1:00</button>
            <button onClick={resetAll} className="px-3 py-2 border rounded-md">Zerar</button>
            <button onClick={saveRound} className="px-3 py-2 border rounded-md bg-gray-900 text-white">Salvar resultado</button>
          </div>
        </div>
        {!selected ? (
      ...
      <section className="card p-3 md:p-5">
        ...
        <div className="sheet">
          <table>
            <thead><tr><th className="w-28"></th>{(Object.keys(DESAFIOS) as DKey[]).map(k=><th key={k}>{DESAFIOS[k].title}</th>)}<th className="w-28">SOMA</th></tr></thead>
            <tbody>
              {Array.from({length:maxRows}).map((_,r)=>(
                <tr key={r}>
                  <td></td>
                  {(Object.keys(DESAFIOS) as DKey[]).map(k=>{
                    const cfg=DESAFIOS[k]; const exists=r<cfg.rows; const on=exists?tab[k][r]:false;
                    return <td key={k+r}>{exists? <button className={cellCls(on)} onClick={()=>toggle(k,r)} disabled={!canScore} title={`${cfg.points} pontos`}>{on?cfg.points:""}</button>: <div className="cell-btn" />}</td>;
                  })}
                  <td><div className="cell-btn" /></td>
                </tr>
              ))}
            </tbody>
            <tfoot><tr><th>SOMA</th>{(Object.keys(DESAFIOS) as DKey[]).map(k=><td key={"s"+k} className="summary">{somaCol[k]}</td>)}<td className="summary">{somaDes}</td></tr></tfoot>
          </table>
        </div>
      </section>
      ...
      <section className="card p-3 md:p-5">
        ...
        <div className="sheet">
          <table>
            <thead><tr><th>Marcador</th><th>Tentativa 1 (100)</th><th>Tentativa 2 (75)</th><th>Tentativa 3 (50)</th><th>SOMA</th></tr></thead>
            <tbody>
              {[1,2].map(m=>(
                <tr key={"m"+m}>
                  <td className="font-medium">{`Marcador ${m}`}</td>
                  {[1,2,3].map(t=>{
                    const ativo = (mark[t as Attempt]===m);
                    return (
                      <td key={`m${m}t${t}`}>
                        <button className={`cell-btn ${ativo?"is-on":"is-off"} ${canScore?"":"opacity-50 cursor-not-allowed"}`} onClick={()=>pick(t as Attempt, ativo?0:m)}>
                          {ativo?MARC[t as 1|2|3]:""}
                        </button>
                      </td>
                    );
                  })}
                  <td><div className="cell-btn"></div></td>
                </tr>
              ))}
            </tbody>
            <tfoot><tr><th>SOMA</th><td className="summary">{mark[1]?100:0}</td><td className="summary">{mark[2]?75:0}</td><td className="summary">{mark[3]?50:0}</td><td className="summary">{([1,2,3] as Attempt[]).reduce((a,t)=>a+(mark[t]?MARC[t]:0),0)}</td></tr></tfoot>
          </table>
        </div>
      </section>
      ...
      <section className="card p-3 md:p-5">
        ...
        <div className="sheet">
          <table>
            <thead><tr><th>Mina</th><th>Tentativa 1 (1,5x)</th><th>Tentativa 2 (1,25x)</th><th>Tentativa 3 (1,15x)</th><th>Multiplicador</th></tr></thead>
            <tbody>
              <tr>
                <td className="font-medium">Mina</td>
                {[1,2,3].map(t=>{
                  const ativo = (mina===t);
                  return <td key={`mina${t}`}><button className={`cell-btn ${ativo?"is-on":"is-off"}`} onClick={()=>setMina(ativo?0:t as 1|2|3)}>{MINA[t as 1|2|3]}</button></td>;
                })}
